...
function getFilteredProperties(properties: SampleProperty[], activeCategory: (typeof CATEGORY_TABS)[number], filters: PropertyFiltersState) {
	...
	return properties.filter((property) => {
		...
		if (keyword && !`${property.title} ${property.location ?? ""}`.toLowerCase().includes(keyword)) return false;
...
export function ProjectListPageClient() {
	...
	const filteredProperties = useMemo(() => getFilteredProperties(SAMPLE_PROPERTIES, activeCategory, appliedFilters), [activeCategory, appliedFilters]);
				...
				<div className={`mt-7 grid gap-6 ${showMapPreview ? "lg:grid-cols-[minmax(0,1fr)_380px]" : ""}`}>
					<div>
						{filteredProperties.length > 0 ? (
							<PropertyGrid properties={filteredProperties} className={showMapPreview ? "grid grid-cols-2 gap-3 sm:gap-5" : undefined} />
						) : (
