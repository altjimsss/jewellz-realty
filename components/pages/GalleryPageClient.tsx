...
const MIN_GALLERY_PHOTOS = 4;

// Ensure every section has enough photos for the fan to actually animate.
// CMS photos stay first (front card); thin sections are padded with photos
// from the matching built-in showcase category (falling back to any showcase
// photo) so Prev/Next always have something to cycle.
function padOptions(options: Option[]): Option[] {
  return options.map((option) => {
    if (option.photos.length >= MIN_GALLERY_PHOTOS) return option;

    const seen = new Set(option.photos.map((photo) => photo.src));
    const sameCategory = OPTIONS.find(
      (showcase) => showcase.title.toLowerCase() === option.title.toLowerCase()
    );
    const pool = sameCategory?.photos ?? OPTIONS.flatMap((showcase) => showcase.photos);
    const extra: Photo[] = [];

    for (const photo of pool) {
      if (extra.length >= MIN_GALLERY_PHOTOS - option.photos.length) break;
      if (seen.has(photo.src)) continue;
      seen.add(photo.src);
      extra.push({ ...photo, label: `${option.title} – ${photo.label}` });
    }

    return extra.length ? { ...option, photos: [...option.photos, ...extra] } : option;
  });
}
...
function groupGalleryItems(items: CmsGalleryItem[]): Option[] {
  ...
  return Array.from(grouped.entries()).map(([section, sectionItems]) => ({
    ...
    photos: dedupePhotos(sectionItems.map((item) => ({
      src: item.imageUrl,
      label: item.title,
      title: item.title,
...
export default function GalleryPage() {
  ...
  const galleryOptions = padOptions(cmsOptions.length ? cmsOptions : OPTIONS);
  ...
  const safeActiveIndex = galleryOptions.length ? Math.min(activeIndex, galleryOptions.length - 1) : 0;
  const activeOption = useMemo(() => galleryOptions[safeActiveIndex] ?? galleryOptions[0] ?? OPTIONS[0], [safeActiveIndex, galleryOptions]);
  const activePhotos = useMemo(() => dedupePhotos(activeOption.photos), [activeOption]);
  const safeActivePhotoIndex = activePhotos.length ? Math.min(activePhotoIndex, activePhotos.length - 1) : 0;
        ...
        <section className="relative min-h-[620px] overflow-hidden border-r border-zinc-200 bg-white px-0 py-0 text-zinc-900">
          ...
          <div className="relative z-10 grid min-h-[600px] grid-cols-1 lg:grid-cols-[0.72fr_1.28fr]">
                  ...
                  <div className="mt-2 flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-[11px] uppercase tracking-[0.16em] text-zinc-500">
                        {activePhotos[safeActivePhotoIndex]?.date ?? activeOption.title}
                      </p>
                      <h3 className="mt-1 text-[16px] font-semibold leading-tight text-zinc-900">
                        {activePhotos[safeActivePhotoIndex]?.title ?? activePhotos[safeActivePhotoIndex]?.label}
                      </h3>
                      <p className="mt-1 text-[12px] leading-6 text-zinc-600">
                        {activePhotos[safeActivePhotoIndex]?.description ?? activeOption.description}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="relative min-h-[400px] overflow-hidden lg:min-h-[400px]">
              <div className="absolute bottom-[-130px] right-[-390px] flex justify-end sm:right-[-370px] lg:right-[-410px]">
                <CardSwap
                  ref={cardSwapRef}
                  width={cardWidth || 720}
                ...
                >
                  {activePhotos.map((photo) => (
                    <Card key={photo.src} customClass="relative">
                      <div className="relative h-full w-full overflow-hidden rounded-[20px] bg-zinc-100">
                        <Image src={photo.src} alt={photo.label} fill loading="eager" className="object-cover" sizes="(max-width: 768px) 100vw, 760px" />
